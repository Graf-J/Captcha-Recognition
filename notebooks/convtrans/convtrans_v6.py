...
wandb.init(
    ...
    name="Convolutional-Transformer:v6",
    config={
        "learning_rate": 0.0001,
        "architecture": "ConvTrans_V1",
        "dataset": "combined-images-augmented",
        "epochs": 200,
        "batch_size": 128,
        "optimizer": "Adam",
...
num_samples = min(70000, len(hf_dataset))
...
# Save the file
with open("test_file_list_convtrans_v6.txt", "w") as f:
    for path in test_paths:
        f.write(path + "\n")

print(f"Saved {len(test_paths)} pointers to test_file_list_convtrans_v6.txt")

# In[6]:


device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = Captcha_Convolutional_Transformer_V1(num_chars=len(generated_dataset.vocab) + 1)  # Add Blank Character to Vocabulary
state_dict = torch.load(
    Captcha_Convolutional_Transformer_V1.SAVE_DIR / "v4.pth", 
    map_location=device,
    weights_only=True 
)
...
model.to(device)
model.train()
print("Successfully loaded v4 weights for fine-tuning!")
...
for epoch in range(num_epochs):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(model.state_dict(), model.SAVE_DIR / "v6.pth")
